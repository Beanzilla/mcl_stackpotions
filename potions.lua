...
stackpot.register_stackpotion = function (def)
    ...
    stackpot.make_convert("mcl_potions:"..def.name, "mcl_stackpotions:"..def.name)
	minetest.register_craftitem("mcl_stackpotions:"..def.name, {
		description = desc,
		_tt_help = basic_potion_tt,
		_doc_items_longdesc = potion_longdesc,
		_doc_items_usagehelp = potion_usagehelp,
		stack_max = def.stack_max or stackpot.STACKSIZE,
	...
	if def.color and not def.no_throwable then
		...
		stackpot.register_splash(def.name, desc, def.color, splash_def)
		if def.description_lingering then
			desc = def.description_lingering
		else
			desc = S("Lingering @1 Potion", def.description)
		end
		stackpot.register_lingering(def.name, desc, def.color, ling_def)
	...
	if def.is_II then
        ...
        stackpot.make_convert("mcl_potions:"..def.name.."_2", "mcl_stackpotions:"..def.name.."_2")
		minetest.register_craftitem("mcl_stackpotions:"..def.name.."_2", {
		...
		if def.color and not def.no_throwable then
			stackpot.register_stacksplash(def.name.."_2", S("Splash @1@2 Potion", def.description, desc_mod), def.color, splash_def_2)
			stackpot.register_stacklingering(def.name.."_2", S("Lingering @1@2 Potion", def.description, desc_mod), def.color, ling_def_2)
			--if not def.no_arrow then
	...
	if def.is_plus then
        ...
        stackpot.make_convert("mcl_potions:"..def.name.."_plus", "mcl_stackpotions:"..def.name.."_plus")
		minetest.register_craftitem("mcl_stackpotions:"..def.name.."_plus", {
			description = S("@1 + Potion", def.description),
			_tt_help = get_tt(def._tt_plus, def.effect, dur_pl),
			_doc_items_longdesc = potion_longdesc,
			_doc_items_usagehelp = potion_usagehelp,
			stack_max = stackpot.STACKSIZE,
			inventory_image = def.image or potion_image(def.color),
			wield_image = def.image or potion_image(def.color),
			groups = def.groups or {brewitem=1, food=3, can_eat_when_full=1},
			on_place = on_use,
		...
		if def.color and not def.no_throwable then
			stackpot.register_splash(def.name.."_plus", S("Splash @1 + Potion", def.description), def.color, splash_def_pl)
			stackpot.register_lingering(def.name.."_plus", S("Lingering @1 + Potion", def.description), def.color, ling_def_pl)
			--if not def.no_arrow then
			--	stackpot.register_arrow(def.name.."_plus", S("Arrow of @1 +", def.description), def.color, arrow_def_pl)
			--end
		end
...
for _, def in ipairs(defs) do
	stackpot.register_stackpotion(def)
end
